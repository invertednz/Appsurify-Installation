        ...
        for parent in parents:
            ...
            if len(output_diff) == len(output_diff_stats):
                for i in range(0, len(output_diff)):
                    ...
                    file_blame = get_commit_blame(file_diff_stats[2].strip(), props.get('sha'), file_diff)
                    print file_blame
                    commit_file = {
...
def get_commit_blame(filename, rev, patch):
    blame = list()
        ...
        if '@@' in string:
            try:
                current_string_number = abs(
                    int(string.split(' @@ ')[0].split('@@ ')[-1].split(' ')[0].split(',')[0]))
            except:
                continue
        else:
    ...
    for string_group in groups:
        ...
        try:
            ...
            blame_results = git(['blame', '-C', rev, '-L', '{},{}'.format(string_group[0], string_group[-1]), '-p', '--', filename])
            blame.append(blame_results)
            # print repr(blame_results)
        except Exception:
            continue
    return blame
...
if __name__ == '__main__':
    ### DEBUG ###
    old, new, ref = sys.argv[1], sys.argv[2], sys.argv[3]
